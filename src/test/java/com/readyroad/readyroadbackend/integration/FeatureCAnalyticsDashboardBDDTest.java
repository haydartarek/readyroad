...
public class FeatureCAnalyticsDashboardBDDTest extends BaseIntegrationTest {
    ...
    void setUp() {
        ...
        // No need to create 150 questions manually - 120 is enough for 2 consecutive exams
    ...
    class StoryC1LearningAnalyticsDashboard {
        ...
        void newUserViewsDashboardWithZeroActivity() {
            // Given: User has not answered any practice questions and not completed any exams
        ...
        void userViewsDashboardAfterPracticeActivity() {
            ...
            // Note: Category breakdown is available via getCategoryProgress() service method
            CategoryProgressResponse categoryProgress = progressService.getCategoryProgress(testUserId).get(0);
            assertThat(categoryProgress.getCategoryName()).isEqualTo(testCategory1.getNameEn()); // Use actual seeded category name
            ...
            if (dashboard.getWeakCategories().size() > 1) {
                for (int i = 0; i < dashboard.getWeakCategories().size() - 1; i++) {
                    assertThat(dashboard.getWeakCategories().get(i).getAccuracy())
                            .isLessThanOrEqualTo(dashboard.getWeakCategories().get(i + 1).getAccuracy());
                }
            }
            ...
            if (dashboard.getStrongCategories().size() > 1) {
                for (int i = 0; i < dashboard.getStrongCategories().size() - 1; i++) {
                    assertThat(dashboard.getStrongCategories().get(i).getAccuracy())
                            .isGreaterThanOrEqualTo(dashboard.getStrongCategories().get(i + 1).getAccuracy());
                }
            }
        }
        ...
        void dashboardShowsTimeWindowedTrendData() {
            ...
            // Time-series data would be in a separate endpoint
            OverallProgressResponse dashboard = progressService.getOverallProgress(testUserId);
        ...
        void dashboardShowsStreakAndLastActivity() {
            ...
            // And: studyStreak should be 1 (current implementation returns 1 for active practice)
    ...
    class StoryC2ExamAnalytics {
        ...
        void userSeesPassFailTrendAcrossMultipleExams() throws Exception {
            ...
            ExamSimulation exam1 = createAndCompleteExam(testUserId, 39); // 78% - FAIL
            Thread.sleep(100); // Ensure different timestamps

            ExamSimulation exam2 = createAndCompleteExam(testUserId, 41); // 82% - PASS
            Thread.sleep(100);

            ExamSimulation exam3 = createAndCompleteExam(testUserId, 30); // 60% - FAIL
            ...
            String trend = completedExams.get(0).getScorePercentage() <
                    completedExams.get(1).getScorePercentage() ? "DECLINING" : "IMPROVING";
