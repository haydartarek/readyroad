-- Final guard for traffic-sign image paths.
-- The only official image source is readyroad/public/images/signs, exposed as /images/signs/**.

CREATE TEMPORARY TABLE allowed_traffic_sign_image_paths (
    image_path VARCHAR(768) PRIMARY KEY
);

INSERT INTO allowed_traffic_sign_image_paths (image_path) VALUES
("/images/signs/danger_signs/A11 Uitweg op een kaai of een oever.png"),
("/images/signs/danger_signs/A13 Dwarse uitholling of ezelsrug.png"),
("/images/signs/danger_signs/A14 Verhoogde inrichting.png"),
("/images/signs/danger_signs/A15 Glibberige rijbaan.png"),
("/images/signs/danger_signs/A17 Kiezelprojectie.png"),
("/images/signs/danger_signs/A19 Vallende stenen.png"),
("/images/signs/danger_signs/A1a Gevaarlijke bocht naar links.png"),
("/images/signs/danger_signs/A1b Gevaarlijke bocht naar rechts.png"),
("/images/signs/danger_signs/A1c Gevaarlijke dubbele of meer dan twee bochten, de eerste naar links.png"),
("/images/signs/danger_signs/A1d Gevaarlijke dubbele of meer dan twee bochten, de eerste naar rechts.png"),
("/images/signs/danger_signs/A21 Oversteekplaats voor voetgangers.png"),
("/images/signs/danger_signs/A23 Plaats waar speciaal veel kinderen komen.png"),
("/images/signs/danger_signs/A25 Oversteekplaats voor fietsers en bromfietsers.png"),
("/images/signs/danger_signs/A27 Overstekend groot wild.png"),
("/images/signs/danger_signs/A29 Overstekend vee.png"),
("/images/signs/danger_signs/A3 Gevaarlijke daling.png"),
("/images/signs/danger_signs/A31 Werken.png"),
("/images/signs/danger_signs/A33 Verkeerslichten.png"),
("/images/signs/danger_signs/A35 Vliegtuigen op geringe hoogte.png"),
("/images/signs/danger_signs/A37 Zijwind.png"),
("/images/signs/danger_signs/A39 Twee richtingsverkeer toegelaten na een stuk éénrichtingsverkeer.png"),
("/images/signs/danger_signs/A41 Overweg met slagbomen.png"),
("/images/signs/danger_signs/A43 Overweg zonder slagbomen.png"),
("/images/signs/danger_signs/A45 Overweg voor enkel spoor.png"),
("/images/signs/danger_signs/A47 Overweg voor twee of meer sporen.png"),
("/images/signs/danger_signs/A49 Kruising van een openbare weg door een of meer in de rijbaan aangelegde sporen.png"),
("/images/signs/danger_signs/A5 Steile helling.png"),
("/images/signs/danger_signs/A50 Opgelet file.png"),
("/images/signs/danger_signs/A51 Gevaar dat niet door een speciaal symbool wordt bepaald.png"),
("/images/signs/danger_signs/A53 Verzinkbare paaltjes.png"),
("/images/signs/danger_signs/A7a Rijbaanversmalling.png"),
("/images/signs/danger_signs/A7b Rijbaanversmalling langs links.png"),
("/images/signs/danger_signs/A7c Rijbaanversmalling langs rechts.png"),
("/images/signs/danger_signs/A9 Beweegbare brug.png"),
("/images/signs/information_signs/F101a Einde voorbehouden voor het verkeer van voetgangers, fietsers, ruiters en bestuurders van speed pedelecs.png"),
("/images/signs/information_signs/F101b Einde deel van de openbare weg voorbehouden voor fietsers en voetgangers.png"),
("/images/signs/information_signs/F101c Einde voorbehouden voor het verkeer van landbouwvoertuigen, voetgangers, fietsers, ruiters en bestuurders van speed pedelecs.png"),
("/images/signs/information_signs/F11 Einde van de autoweg.png"),
("/images/signs/information_signs/F111 Fietsstraat.png"),
("/images/signs/information_signs/F113 Einde fietsstraat.png"),
("/images/signs/information_signs/F12a Begin van een woonerf of van een erf.png"),
("/images/signs/information_signs/F12b Einde van een woonerf of van een erf.png"),
("/images/signs/information_signs/F13 Rijstrook keuze.png"),
("/images/signs/information_signs/F14 Opstelvak voor fietsers en bromfietsen.png"),
("/images/signs/information_signs/F17 Rijstrook aanduiding voorbehouden voor autobussen.png"),
("/images/signs/information_signs/F18 Bijzondere overrijdbare bedding.png"),
("/images/signs/information_signs/F19 Eenrichtingsverkeer.png"),
("/images/signs/information_signs/F1b Begin van een bebouwde kom.png"),
("/images/signs/information_signs/F21 Rechts of links voorbijrijden toegelaten.png"),
("/images/signs/information_signs/F23a Nummer van een gewone weg.png"),
("/images/signs/information_signs/F23b Nummer van een autosnelweg.png"),
("/images/signs/information_signs/F23c Nummer van een internationale weg.png"),
("/images/signs/information_signs/F23d Nummer van een ringweg.png"),
("/images/signs/information_signs/F3b Einde van een bebouwde kom.png"),
("/images/signs/information_signs/F43 Gemeentegrens.png"),
("/images/signs/information_signs/F45 Doodlopende weg.png"),
("/images/signs/information_signs/F45L Doodlopende weg, linkse doorgang.png"),
("/images/signs/information_signs/F45R Doodlopende weg, rechtse doorgang.png"),
("/images/signs/information_signs/F47 Einde van de werken.png"),
("/images/signs/information_signs/F49 Oversteekplaats voor voetgangers.png"),
("/images/signs/information_signs/F5 Autosnelweg.png"),
("/images/signs/information_signs/F50 Oversteekplaats voor fietsers en bromfietsers.png"),
("/images/signs/information_signs/F50-bis Opgepast als je van richting verandert voetgangers.png"),
("/images/signs/information_signs/F50bis Opgepast als je van richting veranderd, fietsers.png"),
("/images/signs/information_signs/F7 Einde autosnelweg.png"),
("/images/signs/information_signs/F8 Tunnel.png"),
("/images/signs/information_signs/F9 Autoweg.png"),
("/images/signs/information_signs/F97 Rijstrook versmalling.png"),
("/images/signs/information_signs/F99a Voorbehouden voor het verkeer van voetgangers, fietsers, ruiters en bestuurders van speed pedelecs.png"),
("/images/signs/information_signs/F99b Deel van de openbare weg voorbehouden voor fietsers en voetgangers.png"),
("/images/signs/information_signs/F99c Voorbehouden voor het verkeer van landbouwvoertuigen, voetgangers, fietsers, ruiters en bestuurders van speed pedelecs.png"),
("/images/signs/mandatory_signs/D10 Deel van de weg voorbehouden voor voetgangers en fietsers.png"),
("/images/signs/mandatory_signs/D11 Verplichte weg voor voetgangers.png"),
("/images/signs/mandatory_signs/D13 Verplichte weg voor ruiters.png"),
("/images/signs/mandatory_signs/D1a Verplichting rechtdoor.png"),
("/images/signs/mandatory_signs/D1b Verplichting links afslaan.png"),
("/images/signs/mandatory_signs/D1b Verplichting rechts afslaan.png"),
("/images/signs/mandatory_signs/D1c Verplichting links aanhouden.png"),
("/images/signs/mandatory_signs/D1d Verplichting rechts aanhouden.png"),
("/images/signs/mandatory_signs/D1e Verplicht links afslaan.png"),
("/images/signs/mandatory_signs/D1f Verplicht rechts afslaan.png"),
("/images/signs/mandatory_signs/D3a Verplicht één van de pijlen te volgen.png"),
("/images/signs/mandatory_signs/D3b Verplicht één van de pijlen te volgen.png"),
("/images/signs/mandatory_signs/D4 Verplicht links afslaan gevaarlijke goederen.png"),
("/images/signs/mandatory_signs/D4 Verplicht rechtdoor gevaarlijke goederen.png"),
("/images/signs/mandatory_signs/D4 Verplicht rechts afslaan gevaarlijke goederen.png"),
("/images/signs/mandatory_signs/D5 Verplicht rondgaand verkeer.png"),
("/images/signs/mandatory_signs/D7 Verplicht fietspad.png"),
("/images/signs/mandatory_signs/D9a Deel van de weg voorbehouden voor voetgangers en fietsers.png"),
("/images/signs/parking_signs/E1 Parkeerverbod.png"),
("/images/signs/parking_signs/E3 Stilstaan en parkeren verboden.png"),
("/images/signs/parking_signs/E5 Parkeerverbod van de 1e tot de 15e van de maand.png"),
("/images/signs/parking_signs/E7 Parkeerverbod van de 16e tot het einde van de maand.png"),
("/images/signs/parking_signs/E9a Parkeren toegelaten.png"),
("/images/signs/parking_signs/E9a-elektrisch laden Parkeerplaats voorbehouden voor het elektrisch opladen van je wagen.png"),
("/images/signs/parking_signs/E9a-mindervaliden Parkeren enkel toegelaten voor mindervaliden.png"),
("/images/signs/parking_signs/E9a-parkeerschijf Parkeren beperkt in tijd, parkeerschijf verplicht.png"),
("/images/signs/parking_signs/E9b Parkeren uitsluitend voor autos.png"),
("/images/signs/parking_signs/E9c Parkeren uitsluitend voor vrachtwagens.png"),
("/images/signs/parking_signs/E9d Parkeren uitsluitend voor autocars.png"),
("/images/signs/parking_signs/E9e Verplicht parkeren op de berm of op het trottoir.png"),
("/images/signs/parking_signs/E9f Verplicht parkeren deels op de berm of op het trottoir.png"),
("/images/signs/parking_signs/E9g Verplicht parkeren op de rijbaan.png"),
("/images/signs/parking_signs/E9j wisselend parkeren Parkeerplaats voorzien voor wisselend parkeren fietsers en autos.png"),
("/images/signs/priority_signs/B1 Voorrang verlenen.png"),
("/images/signs/priority_signs/B11 Einde van voorrangsweg.png"),
("/images/signs/priority_signs/B15a Voorrang op het eerstvolgende kruispunt - variant schuine zijweg links.png"),
("/images/signs/priority_signs/B15b Voorrang op het eerstvolgende kruispunt - variant schuine zijweg rechts.png"),
("/images/signs/priority_signs/B15c Voorrang op het eerstvolgende kruispunt.png"),
("/images/signs/priority_signs/B15d Voorrang op het eerstvolgende kruispunt - variant zijweg links.png"),
("/images/signs/priority_signs/B15e Voorrang op het eerstvolgende kruispunt - variant T-kruispunt.png"),
("/images/signs/priority_signs/B15f Voorrang op het eerstvolgende kruispunt - variant Y-kruispunt.png"),
("/images/signs/priority_signs/B15g Voorrang op kruisende zijweg.png"),
("/images/signs/priority_signs/B17 Kruispunt waar de voorrang van rechts geldt.png"),
("/images/signs/priority_signs/B19 Smalle doorgang. Voorrang verlenen aan de bestuurders die uit de tegenovergestelde richting komen.png"),
("/images/signs/priority_signs/B21 Smalle doorgang. Voorrang ten opzichte van de bestuurders die uit de tegenovergestelde richting komen.png"),
("/images/signs/priority_signs/B22 Fietsers en speed pedelecs mogen rechts afslaan en de verkeerslichten voorbijrijden.png"),
("/images/signs/priority_signs/B23 Fietsers en speed pedelecs mogen rechtdoor rijden en de verkeerslichten voorbijrijden.png"),
("/images/signs/priority_signs/B5 Stop en voorrang verlenen.png"),
("/images/signs/priority_signs/B9 Voorrangsweg.png"),
("/images/signs/prohibition_signs/C1 Verboden richting voor iedere bestuurder.png"),
("/images/signs/prohibition_signs/C11 Verboden toegang voor bestuurders van rijwielen.png"),
("/images/signs/prohibition_signs/C13 Verboden toegang voor bestuurders van gespannen.png"),
("/images/signs/prohibition_signs/C15 Verboden toegang voor ruiters.png"),
("/images/signs/prohibition_signs/C17 Verboden toegang voor bestuurders van handkarren.png"),
("/images/signs/prohibition_signs/C19 Verboden toegang voor voetgangers.png"),
("/images/signs/prohibition_signs/C21 Verboden toegang voor voertuigen met een massa groter dan aangeduid.png"),
("/images/signs/prohibition_signs/C22 Verboden toegang voor bestuurders van autocars.png"),
("/images/signs/prohibition_signs/C23 Verboden toegang voor bestuurders van voertuigen bestemd of gebruikt voor het vervoer van zaken.png"),
("/images/signs/prohibition_signs/C24a Verboden toegang voor bestuurders van voertuigen die gevaarlijke goederen vervoeren.png"),
("/images/signs/prohibition_signs/C24b Verboden toegang voor bestuurders van voertuigen die gevaarlijke ontvlambare of ontplofbare stoffen vervoeren.png"),
("/images/signs/prohibition_signs/C24c Verboden toegang voor bestuurders van voertuigen die gevaarlijke verontreinigende stoffen vervoeren.png"),
("/images/signs/prohibition_signs/C25 Verboden voor voertuigen langer dan het aangeduide.png"),
("/images/signs/prohibition_signs/C27 Verboden voor voertuigen breder dan het aangeduide.png"),
("/images/signs/prohibition_signs/C29 Verboden voor voertuigen hoger dan het aangeduide.png"),
("/images/signs/prohibition_signs/C3 Verboden toegang in beide richtingen voor iedere bestuurder.png"),
("/images/signs/prohibition_signs/C31a Verbod om links af te slaan.png"),
("/images/signs/prohibition_signs/C31b Verbod rechts af te slaan.png"),
("/images/signs/prohibition_signs/C33 Verbod om te keren.png"),
("/images/signs/prohibition_signs/C35 Verbod een voertuig links in te halen.png"),
("/images/signs/prohibition_signs/C37 Einde verbod opgelegd door het verkeersbord C35.png"),
("/images/signs/prohibition_signs/C39 Verbod voertuigen met toegelaten massa groter dan 3500 kg in te halen.png"),
("/images/signs/prohibition_signs/C41 Einde van het verbod opgelegd door het verkeersbord C39.png"),
("/images/signs/prohibition_signs/C43 Verbod te rijden met een grotere snelheid dan 50 km.png"),
("/images/signs/prohibition_signs/C45 Einde van de snelheidsbeperking opgelegd door het verkeersbord C43.png"),
("/images/signs/prohibition_signs/C46 Einde van alle plaatselijke verbodsbepalingen opgelegd aan de voertuigen in beweging.png"),
("/images/signs/prohibition_signs/C47 Tolpost. Verbod voorbij te rijden zonder te stoppen.png"),
("/images/signs/prohibition_signs/C5 Verboden toegang voor bestuurders van motorvoertuigen met meer dan twee wielen.png"),
("/images/signs/prohibition_signs/C5-C7 Verboden toegang voor motorvoertuigen en motorfietsen.png"),
("/images/signs/prohibition_signs/C6 Verboden toegang voor bestuurders van quads.png"),
("/images/signs/prohibition_signs/C7 Verboden toegang voor bestuurders van motorfietsen.png"),
("/images/signs/prohibition_signs/C9 Verboden toegang voor bestuurders van bromfietsen.png"),
("/images/signs/prohibition_signs/C9-C11 Verboden toegang voor bestuurders van bromfietsen en fietsen.png"),
("/images/signs/road_markings/F39 Aankondiging van een omleiding.png"),
("/images/signs/road_markings/F79 Tijdelijke verdeling van de rijstroken (met afstandsaanduiding).png"),
("/images/signs/road_markings/F81 Voorwegwijzer uitwijking.png"),
("/images/signs/road_markings/F83 Versmalling van de rijbaan.png"),
("/images/signs/road_markings/F85 Verlegging van de rijbaan.png"),
("/images/signs/road_markings/F89 Aanduiding van de maximumsnelheid per rijstrook.png"),
("/images/signs/road_markings/F91 Aanduiding van de maximumsnelheid per rijstrook (zonder afstand).png"),
("/images/signs/road_markings/F95 Einde van een rijstrook.png"),
("/images/signs/road_markings/F98 Bijzondere rijstrookregeling.png"),
("/images/signs/zone_signs/F103 Begin van een voetgangerszone.png"),
("/images/signs/zone_signs/F105-Einde zone van een voetgangerszone.png"),
("/images/signs/zone_signs/F117 Begin van lage emissiezone.png"),
("/images/signs/zone_signs/F118-Einde van lage emissiezone.png"),
("/images/signs/zone_signs/ZC21 Zone verboden toegang voor bestuurders van voertuigen waarvan de massa hoger dan 3500 kg.png"),
("/images/signs/zone_signs/ZC21-Einde zone verboden toegang voor bestuurders van voertuigen waarvan de massa hoger dan 3500 kg.png"),
("/images/signs/zone_signs/ZC35 Zone verboden inhalen.png"),
("/images/signs/zone_signs/ZC35-Einde zone verboden inhalen.png"),
("/images/signs/zone_signs/ZC43 Zone met een snelheidsbeperking.png"),
("/images/signs/zone_signs/ZC45-Einde zone met een snelheidsbeperking.png"),
("/images/signs/zone_signs/ZC5 Zone verboden toegang voor motorvoertuigen met meer dan 2 wielen.png"),
("/images/signs/zone_signs/ZC5-Einde zone verboden toegang voor motorvoertuigen met meer dan 2 wielen.png"),
("/images/signs/zone_signs/ZE1 Zone parkeerverbod.png"),
("/images/signs/zone_signs/ZE1-Einde zone parkeerverbod.png"),
("/images/signs/zone_signs/ZE9a Zone parkeren uitsluitend voor auto's.png"),
("/images/signs/zone_signs/ZE9a-Einde zone parkeren uitsluitend voor auto's.png"),
("/images/signs/zone_signs/ZE9aT Zone parkeren uitsluitend voor auto's.png"),
("/images/signs/zone_signs/ZE9aT-Einde zone parkeren uitsluitend voor auto's.png"),
("/images/signs/zone_signs/Zone-F111 Zone Fietsstraat.png"),
("/images/signs/zone_signs/Zone-F111-Eind zone Fietsstraat.png"),
("/images/signs/zone_signs/Zone-F4a Zone 30 km.png"),
("/images/signs/zone_signs/Zone-F4b-Einde zone 30 km.png");

UPDATE road_signs
SET image_path = CONCAT('/', image_path)
WHERE image_path LIKE 'images/signs/%';

UPDATE exam_questions
SET image_url = CONCAT('/', image_url)
WHERE image_url LIKE 'images/signs/%';

UPDATE quiz_questions
SET content_image_url = CONCAT('/', content_image_url)
WHERE content_image_url LIKE 'images/signs/%';

UPDATE lessons
SET icon = CONCAT('/', icon)
WHERE icon LIKE 'images/signs/%';

UPDATE road_signs
SET image_path = NULL
WHERE image_path IS NOT NULL
  AND image_path <> ''
  AND image_path NOT LIKE '/images/signs/%';

UPDATE road_signs rs
LEFT JOIN allowed_traffic_sign_image_paths allowed
    ON allowed.image_path = rs.image_path
SET rs.image_path = NULL
WHERE rs.image_path LIKE '/images/signs/%'
  AND allowed.image_path IS NULL;

UPDATE exam_questions eq
LEFT JOIN allowed_traffic_sign_image_paths allowed
    ON allowed.image_path = eq.image_url
SET eq.image_url = NULL
WHERE eq.image_url LIKE '/images/signs/%'
  AND allowed.image_path IS NULL;

UPDATE quiz_questions qq
LEFT JOIN allowed_traffic_sign_image_paths allowed
    ON allowed.image_path = qq.content_image_url
SET qq.content_image_url = NULL
WHERE qq.content_image_url LIKE '/images/signs/%'
  AND allowed.image_path IS NULL;

UPDATE lessons l
LEFT JOIN allowed_traffic_sign_image_paths allowed
    ON allowed.image_path = l.icon
SET l.icon = NULL
WHERE l.icon LIKE '/images/signs/%'
  AND allowed.image_path IS NULL;

DROP TEMPORARY TABLE allowed_traffic_sign_image_paths;
