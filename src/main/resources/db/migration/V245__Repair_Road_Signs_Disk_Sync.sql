-- Reapply the V243 road-sign disk sync for databases where V243 was
-- manually marked as successful after an earlier duplicate-key failure.
-- The statements are idempotent, so fresh databases that already ran
-- the corrected V243 can safely run this as a no-op reconciliation.
